...
function MapPreview() {
  ...
  // Highlighted pulse location (Italy)
  const pulsePoint = useMemo(() => {
    if (!worldData) return null
    const feat = worldData.features.find(f => f.id === '380')
    if (!feat) return null
  ...
  return (
    <div style={{
      position: 'relative', width: '100%', maxWidth: 620, aspectRatio: '16/9',
      background: 'linear-gradient(135deg, #f5f8f5 0%, #eef2ed 100%)', borderRadius: 18, margin: '0 auto',
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: '100%', display: 'block' }}>
        <defs>
          <radialGradient id="oceanGrad" cx="50%" cy="50%" r="75%">
            ...
            <stop offset="100%" stopColor="#e7ede7" />
          </radialGradient>
        </defs>
        ...
        {worldData && worldData.features.map(f => {
          ...
          return (
            <path
              key={f.id}
              d={d}
              fill={isVisited ? '#5B8C5A' : '#e0dccf'}
              stroke={isVisited ? '#ffffff' : 'rgba(58,74,92,0.12)'}
              strokeWidth={isVisited ? 1.0 : 0.3}
              style={isVisited ? { filter: 'drop-shadow(0 1px 2px rgba(91,140,90,0.3))' } : undefined}
            />
          )
        })}
        {pulsePoint && (
          <g transform={`translate(${pulsePoint[0]}, ${pulsePoint[1]})`}>
            <circle r="6" fill="#5B8C5A" opacity="0.35">
              <animate attributeName="r" values="5;14;5" dur="2.2s" repeatCount="indefinite" />
              <animate attributeName="opacity" values="0.5;0;0.5" dur="2.2s" repeatCount="indefinite" />
            </circle>
            <circle r="3.5" fill="#5B8C5A" stroke="#fff" strokeWidth="1.5" />
          </g>
        )}
      </svg>
      <div style={{
        ...
        borderRadius: 14, padding: '11px 22px', display: 'flex', gap: 26,
        border: '1px solid rgba(91,140,90,0.15)',
        boxShadow: '0 6px 24px rgba(58,74,92,0.12)',
      }}>
        {[{ v: '12', l: 'Countries' }, { v: '34', l: 'Cities' }, { v: '4', l: 'Continents' }].map((s, i) => (
          <div key={i} style={{ textAlign: 'center' }}>
            <div style={{ color: '#5B8C5A', fontSize: 22, fontWeight: 800, fontFamily: 'var(--f-display)', lineHeight: 1 }}>{s.v}</div>
            <div style={{ color: 'var(--text-muted)', fontSize: 8, fontWeight: 700, letterSpacing: '0.1em', marginTop: 4 }}>{s.l.toUpperCase()}</div>
          </div>
        ))}
      </div>
    </div>
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ textAlign: 'center', marginTop: 24 }}>
                <button onClick={(e) => { e.stopPropagation(); navigate('/dashboard') }} style={{
                  ...
                  letterSpacing: '0.02em', fontFamily: 'var(--f-body)',
                  boxShadow: '0 4px 16px rgba(91,140,90,0.35)',
                  transition: 'transform 0.2s ease, box-shadow 0.2s ease',
                }}
                onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 6px 22px rgba(91,140,90,0.45)' }}
                onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 16px rgba(91,140,90,0.35)' }}>
                  Open the Map →
                </button>
              </div>
            </div>
